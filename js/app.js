...
function aplicarUsuarioNaUI(u) {
  ...
  // Move XP label to follow bar fill
  const xpValContainer = perfXpVal?.parentElement;
  if (xpValContainer) xpValContainer.style.left = Math.min(xpPct, 90) + '%';
  ...
  tarefas.forEach(t => {
    ...
    card.innerHTML = `
      ...
      ${concluida ? `<div style="display:flex;justify-content:flex-end;margin-top:8px;">
        ${t.tem_post
          ? `<button onclick="event.stopPropagation();navTo('dashboard');setTimeout(()=>rolarParaPost(${t.id}),300)" style="padding:5px 12px;border-radius:8px;border:1px solid #7c3aed;background:transparent;color:#c4b5fd;font-size:11px;font-weight:600;cursor:pointer;">Ver post</button>`
...
async function excluirPostFeed(postId, idx) {
  ...
  if (ok) {
    ...
    await carregarTarefas(); // atualiza tem_post
  } else {
...
async function abrirPerfilUsuario(uid) {
  ...
  if (rankRow) {
    if (data.posicao_ranking) {
      rankRow.innerHTML = `
        <span style="width:58px;font-weight:700;">${data.posicao_ranking}º</span>
        <span style="flex:1;">@${data.nome}</span>
        <span style="width:85px;">${data.xp_ranking} XP</span>
        <span style="width:85px;">${data.tarefas_concluidas} tarefas</span>`;
    } else {
  ...
  // Conquistas
  const conquEl = document.getElementById('mpu-conquistas');
  if (conquEl) conquEl.innerHTML = '<span style="font-size:12px;color:#64748b;">—</span>';
...
// ─────────────────────────────────────────────
// HELPER: rola feed até o post da tarefa
// ─────────────────────────────────────────────
function rolarParaPost(tarefaId) {
  const container = document.getElementById('feed-posts');
  if (!container) return;
  const posts = container.querySelectorAll('.post-card');
  // O feed está em ordem cronológica desc; tenta achar por data_tarefa
  // Por ora apenas rola ao topo do feed
  const feedCol = document.querySelector('.feed-col');
  if (feedCol) feedCol.scrollTop = 0;
  mostrarToast('Veja seu post no feed acima ↑');
}
// ─────────────────────────────────────────────
